...
define([
  ...
  return declare("O.esri.Tiles.OfflineTileEnablerLayer", [TiledMapServerLayer], {
    ...
    getTileUrl: function (level, row, col) {
      ...
      if (this.offline.online && this.offline.store.index.hasKey!="undefined" && !this.offline.store.index.hasKey(url)) {
    ...
    _doNextTile: function (i, cells, reportProgress) {
      ...
      /*Luis*/
      var notifyAndContinue = function (error, success){
        var cancelRequested = reportProgress({
          countNow: i,
          countMax: cells.length,
          cell: cell,
          error: error,
      ...
      if (url.lastIndexOf("void:") == -1) {
        this._tilesCore._storeTile(url, this.offline.proxyPath, this.offline.store, function (success, error) {
          ...
          notifyAndContinue(error, success);

        }.bind(this));
      }
      else {
        console.log("retrieval will be from the offline store...");
        notifyAndContinue(false, true);
...
O.esri.Tiles.TilesStore = function () {
  ...
  this.index =  {
    ...
    remove: function (key) {
      var keyIndex = this.idx.lastIndexOf(key);
      console.log("removing '" + key + "' index");
      if (keyIndex != -1) {
        this.idx.splice(keyIndex, 1);
      }
    },
    ...
    hasKey: function (key) {
        ...
        if (!found) {
          console.log("key "+ key + " not found in the index");
        }
        return found;
    },
    ...
    clear: function () {
      ...
      this.idx.splice(0, this.idx.length);
    }
  };

  this.buildKeyMap = function () {
    if (this._db !== null) {
      var objectStore = this._db.transaction([this.objectStoreName]).objectStore(this.objectStoreName);
      ...
      console.log("the index was loaded: " + this.index.length() + " keys");
    }
  };
   ...
   * @param callback
   */
  this.isInStore = function(url, callback){
      var isIn = this.index.hasKey(url);
  ...
  this.init = function (callback) {
    ...
    callback = callback || function (success) {
      console.log("TilesStore::init() success:", success);
    }.bind(this);
